...
database your pages query. Start from `skills/notes.ts` — it demonstrates
every surface above: two proposal kinds, the tools that propose them, a read
tool that emits a component, two mutations, a widget, and a scheduled task
handler — then register the new skill in `selfctl.config.ts`'s `skills: [...]`
array.
...
See `docs/smoke.md` for the full end-to-end smoke of the protocol surface —
run against a deploy preview rather than locally, since the notes skill's
generated feature image needs Netlify AI Gateway credentials only a deployed
site has (threads, messages, the event log, streaming, the scheduled tick,
plus the notes skill's mutations, widget, and generated asset).
